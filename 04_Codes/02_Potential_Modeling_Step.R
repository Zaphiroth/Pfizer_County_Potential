...
universe_data_js_m <- universe_data_js %>%
  ...
  select(-pha, -specialty.3)

##-- do the roughly miss values impute
universe_data_js_m1 <- universe_data_js_m %>%
...
training_data$value <- (training_data$value_2017) ^ (1 / 4)
...
importance(potential_model, type = 1)
varImpPlot(potential_model, type = 1, sort = TRUE)

training_data$fitted_value <- predict(potential_model)

1 - sum((training_data$value - training_data$fitted_value) ^ 2) /
  sum((training_data$value - mean(training_data$value)) ^ 2)

(mse_rf <- mean((training_data$value - training_data$fitted_value) ^ 2))
(mse_rf_1 <- mean(((training_data$value) ^ 4 - (training_data$fitted_value) ^ 4) ^ 2))
...
testing_data_m$value <- (potential_pred) ^ 4
# testing_data_m$fitted_value <- (potential_pred) ^ 4

training_data_m <- universe_data_js_m1 %>%
  filter(flag == 1)
sum((potential_pred) ^ 4) / (sum((potential_pred) ^ 4) + sum(training_data_m$value_2017))
...
sample_data_m <- county_sample_data_js %>%
...
final_data_chk1 <- final_data %>%
  ...
  summarise(value = sum(value, na.rm = TRUE),
            value_prj = sum(value_prj, na.rm = TRUE)) %>%
  ungroup() %>%
  arrange(desc(value)) %>%
...
saveWorkbook(wb, "county_hospitals_potential_projection_0914.xlsx",
             overwrite = TRUE)
...
formula_m <- as.formula(paste(dependent_var, " ~ ",
                            paste(c(independent_vars, "specialty_2"), collapse = " + ")))

##-- dummy data

dum_var <- dummyVars(formula_m, data = universe_data_js_m1)

dum_universe_data_js <- predict(dum_var, universe_data_js_m1) %>%
  data.frame() %>%
  mutate(hospital_name = universe_data_js_m1$hospital_name,
         value = universe_data_js_m1$value,
         flag = universe_data_js_m1$flag)

dum_training_data <- dum_universe_data_js %>%
  filter(flag == 1) %>%
  mutate(value_s = value ^ (1/2))

dum_testing_data <- dum_universe_data_js %>%
  filter(flag == 0) %>%
  select(-flag, -value)

##-- find lambda

lambda_model <- cv.glmnet(as.matrix(subset(dum_training_data, 
                                           select = c(-value, -value_s, -flag, -hospital_name))), 
                          dum_training_data[,"value_s"], 
                          nfolds = 5, family = "gaussian", type.measure = "mse")
plot(lambda_model)

lambda_model$glmnet.fit
lambda_model$lambda.min
lambda_model$lambda.1se

##-- modeling

potential_model_lasso <- glmnet(as.matrix(subset(dum_training_data, 
                                                 select = c(-value, -value_s, -flag, -hospital_name))), 
                                dum_training_data[,"value_s"], 
                                alpha = 1, 
                                family = "gaussian", 
                                lambda = lambda_model$lambda.min, 
                                intercept = FALSE, 
                                lower.limits = 0)

(coefficients <- coef.glmnet(potential_model_lasso, s = potential_model_lasso$lambda))

##-- initial value modeling

lambda_model_m <- cv.glmnet(as.matrix(subset(dum_training_data, 
                                             select = c(-value, -value_s, -flag, -hospital_name))), 
                            dum_training_data[,"value"], 
                            nfolds = 5, family = "gaussian", type.measure = "mse")
plot(lambda_model_m)

lambda_model_m$glmnet.fit
lambda_model_m$lambda.min
lambda_model_m$lambda.1se

potential_model_lasso_m <- glmnet(as.matrix(subset(dum_training_data, 
                                                   select = c(-value, -value_s, -flag, -hospital_name))), 
                                  dum_training_data[,"value"], 
                                  alpha = 1, 
                                  family = "gaussian", 
                                  lambda = lambda_model_m$lambda.min, 
                                  intercept = FALSE, 
                                  lower.limits = 0)

(coefficients_m <- coef.glmnet(potential_model_lasso_m, s = potential_model_lasso_m$lambda))

##-- evaluating

dum_training_data_pred <- predict(potential_model_lasso, 
                                  as.matrix(subset(dum_training_data, 
                                                   select = c(-value, -value_s, -flag, -hospital_name))))

(mse_lasso <- mean((dum_training_data_pred - dum_training_data$value_s) ^ 2))
(mse_lasso_1 <- mean(((dum_training_data_pred) ^ 4 - dum_training_data$value) ^ 2))

dum_training_data_pred_m <- predict(potential_model_lasso_m, 
                                    as.matrix(subset(dum_training_data, 
                                                     select = c(-value, -value_s, -flag, -hospital_name))))

(mse_lasso_m <- mean((dum_training_data_pred_m - dum_training_data$value) ^ 2))

(r_square <- 
  1 - sum((dum_training_data$value_s - dum_training_data_pred) ^ 2) /
    sum((dum_training_data$value_s - mean(dum_training_data$value_s)) ^ 2))

(r_square_m <- 
  1 - sum((dum_training_data$value - dum_training_data_pred_m) ^ 2) /
    sum((dum_training_data$value - mean(dum_training_data$value)) ^ 2))

mse_lasso / mse_rf
mse_lasso_1 / mse_rf_1
mse_lasso_m / mse_rf_1
mse_lasso_1 / mse_lasso_m

##-- prediction

potential_pred_lasso <- predict(potential_model_lasso, 
                                as.matrix(subset(dum_testing_data, 
                                                 select = c(-value, -hospital_name))))
potential_pred_lasso_m <- predict(potential_model_lasso_m, 
                                  as.matrix(subset(dum_testing_data, 
                                                   select = c(-value, -hospital_name))))

dum_testing_data$value <- (potential_pred_lasso) ^ 2
sum(dum_testing_data$value_trans)

wb_lasso <- createWorkbook()
addWorksheet(wb_lasso, "prediction_results")
addWorksheet(wb_lasso, "coef_trans")
addWorksheet(wb_lasso, "coef")

writeData(wb_lasso, "prediction_results", dum_testing_data)
writeData(wb_lasso, "coef_trans", as.matrix(coefficients), rowNames = TRUE)
writeData(wb_lasso, "coef", as.matrix(coefficients_m), rowNames = TRUE)

saveWorkbook(wb_lasso, "county_hospitals_potential_projection_lasso_0919(1).xlsx",
             overwrite = TRUE)
