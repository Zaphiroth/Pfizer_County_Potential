...
demograph_data_m <- demograph_data %>%
  dplyr::select(-c(permanent_residents_birth_rate, city, 
                   new_born, sample_vaccine_data)) %>%
  mutate(county = ifelse(county == "海安县", "海安市", county))

demograph_data_js <- demograph_data %>% 
  filter(province == "江苏省") %>%
  dplyr::select(-c(province, city,
            permanent_residents_birth_rate,
            new_born, sample_vaccine_data)) %>%
  mutate(county = ifelse(county == "海安县", "海安市", county))
...
htn_county_sample_data_js_m1 <- htn_county_sample_data_js_m %>%
  ...
  #               Spectialty_1, Specialty_2, Est_DrugIncome_RMB, sale,
...
table(htn_county_sample_data_js_m1$city, useNA = 'always')
...
county_hospitals_universe_js <-
  ...
  dplyr::select(-c(Type, Province, City, county.HP, City.Tier.2010, City.Tier.2018,
...
universe_data_js <- htn_county_sample_data_js_m2 %>% 
  mutate(`医生数` = as.numeric(`医生数`)) %>% 
...
universe_data_js_m <- bind_rows(universe_data_js_test,
